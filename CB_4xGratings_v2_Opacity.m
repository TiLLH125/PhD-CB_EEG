%% One-Shot Change Blindness (4 gratings) + Opacity-Adaptive QUEST+ + PAS First in Q1
% Fixed-timing opacity-adaptive version (based on CB_4xGratings_v2.m).
% - 2x2 grid of gratings; fixed S1/ISI/S2/gap/ITI frame durations
...
%   stimulus opacity (%) whose predicted target probability under the posterior MAP is
%   closest to its target:
%       1 Blind       -> P(Blind)         ~= 0.725
%       2 Sensing     -> P(detect)        ~= 0.50
%       3 Seeing      -> P(loc | aware)   ~= 0.65
% In this script, QUEST+ x = opacity percentage.



function CB_4xGratings_v2_Opacity
...
% ---- EEG serial triggers (Phase 3: event + response markers on main trials and Practice Block 2) ----
cfg.eeg.enable            = false;   % default false for safe behavioural testing
cfg.eeg.requirePort       = true;    % if true, abort clearly if EEG serial port cannot open
cfg.eeg.serialPort        = 'COM7';
cfg.eeg.baudRate          = 115200;
cfg.eeg.warnOnSendError   = true;

% Important: avoid blocking reset by default
cfg.eeg.resetMode         = 'none';  % options later: 'none', 'blocking', 'deferred'
cfg.eeg.pulseWidthSec     = 0.005;   % only used if resetMode = 'blocking' later
...
cfg.eeg.codes.pasBase    = 40; % 40 + PAS digit
cfg.eeg.codes.locBase    = 50; % 50 + localisation digit

cfg.eeg.markerPolicy = struct();
cfg.eeg.markerPolicy.markPracticeTrials  = true;
cfg.eeg.markerPolicy.markPracticeBlock1  = false;
...
cfg.practice1.nHARD     = 1;    % hard/faint change trials
cfg.practice1.nTrials   = cfg.practice1.nSTD + cfg.practice1.nNCH + cfg.practice1.nEASY + cfg.practice1.nHARD;

% Opacity by trial type
cfg.practice1.opacitySTD  = 40;   % standard change
cfg.practice1.opacityEASY = 80;   % obvious / easy anchor
cfg.practice1.opacityHARD = 15;   % hard/faint change
cfg.practice1.opacityNCH  = 40;   % no-change (standard opacity)
...
cfg.practice2.nHARD     = 1;
cfg.practice2.nTrials   = cfg.practice2.nSTD + cfg.practice2.nNCH + cfg.practice2.nEASY + cfg.practice2.nHARD;

% Opacity by trial type (fixed timing; same levels as Block 1)
cfg.practice2.opacitySTD  = 40;
cfg.practice2.opacityEASY = 80;
cfg.practice2.opacityHARD = 15;
cfg.practice2.opacityNCH  = 40;
...
cfg.calib.convergenceTolOpacityPct = 1;    % max range in that window for freeze

% In this script, QUEST+ x = opacity percentage.
% Detection threshold grid spans 1-80% opacity; localisation grid spans 10-100%.
cfg.calib.questPlus.alphaDetRange = round(linspace(1, 80, 17));
cfg.calib.questPlus.betaDetRange  = [0.00934 0.0136 0.0198 0.0289 0.0421 0.0614 0.0895];
cfg.calib.questPlus.alphaLocRange = round(linspace(10, 100, 19));
cfg.calib.questPlus.betaLocRange  = [0.00146 0.00246 0.00413 0.00693 0.0116 0.0195 0.0328 0.0551];
cfg.calib.questPlus.lambdaRange   = [0 0.01 0.02 0.04 0.05];
...
cfg.calib.tracks(1).startOpacityPct = 5;    % very faint
...
cfg.calib.tracks(2).startOpacityPct = 30;   % low/mid opacity

cfg.calib.tracks(3).name          = 'Seeing';
cfg.calib.tracks(3).targetOutcome = 'PlocGivenAware';  % P(loc correct | aware); MOCS-informed target
cfg.calib.tracks(3).targetProb    = 0.65;
cfg.calib.tracks(3).startOpacityPct = 60;   % clearer

% Trial counts
cfg.debug.quickTest = true;   % set false for pilot/EEG data collection
cfg.debug.forceFixedOpacity = true;
cfg.debug.fixedOpacityPct   = 80;
cfg.debug.printTrialSummary = true;
cfg.debug.nDebugBlocks      = [];
cfg.debug.nDebugTrials      = [];
cfg.debug.trialLog = true;
cfg.debug.logHeaderEachBlock = true;

if cfg.debug.quickTest
    % Quick debug run only — not for pilot/real EEG data
    cfg.calib.maxTrials = 9;
    cfg.trialsPerBlock  = 3;
    cfg.trialDial.nChangePerBlock   = 2;
    cfg.trialDial.nNoChangePerBlock = 1;
    cfg.trialDial.pChange = 2/3;   % honest 2/3 change (not 75%)
else
    % Pilot/full experiment (configurable; must be divisible by trialsPerBlock and track balance)
    cfg.calib.maxTrials = 600;
    cfg.trialsPerBlock  = 4;
    cfg.trialDial.nChangePerBlock   = 3;
    cfg.trialDial.nNoChangePerBlock = 1;
    cfg.trialDial.pChange = 3/4;   % 75% change
end

cfg.nTotal = cfg.calib.maxTrials;
cfg.trialDial.applyPerBlock = true;

% Apply the dial (sets global nChange/nCatch from pChange)
cfg = applyTrialDial(cfg);

% Debug run length override (optional)
if ~isempty(cfg.debug.nDebugTrials)
    cfg.nTotal = cfg.debug.nDebugTrials;
elseif ~isempty(cfg.debug.nDebugBlocks)
    cfg.nTotal = cfg.debug.nDebugBlocks * cfg.trialsPerBlock;
end
if ~isempty(cfg.debug.nDebugTrials) || ~isempty(cfg.debug.nDebugBlocks)
    cfg = applyTrialDial(cfg);
end
...
cfg.nChangePerTrack = cfg.nChange/3;   % 150 each at default 600 total, 3 change/block
cfg.nCatchPerTrack  = cfg.nCatch/3;    % 50 each at default 600 total, 1 catch/block

% Fixed timing (frames at 120 Hz; not derived from seconds)
cfg.S1_frames        = 72;    % 600 ms at 120 Hz
cfg.ISI_frames       = 18;    % 150 ms at 120 Hz
cfg.S2_frames        = 72;    % 600 ms at 120 Hz
cfg.postS2Gap_frames = 30;    % 250 ms at 120 Hz; Q1 ~850 ms after S2 onset
cfg.ITI_frames       = 120;   % 1000 ms at 120 Hz
cfg.fixJitterFrames  = [120 180];  % 1000-1500 ms at 120 Hz
cfg.maxRespSec        = 30.00;         % failsafe

% Opacity QUEST+ bounds (percentage)
cfg.minOpacityPct = 1;
cfg.maxOpacityPct = 100;
...
outFile = fullfile(outDir, sprintf('CB_4xGratings_Opacity_%s_FullRun_%s.csv', cfg.participantID, timestamp));
...
try
    ...
    fprintf('\nTiming configuration (fixed frames at %.1f Hz):\n', 1/ifi);
    fprintf('  S1_frames        = %d (%.1f ms)\n', cfg.S1_frames, cfg.S1_frames * ifi * 1000);
    fprintf('  ISI_frames       = %d (%.1f ms)\n', cfg.ISI_frames, cfg.ISI_frames * ifi * 1000);
    fprintf('  S2_frames        = %d (%.1f ms)\n', cfg.S2_frames, cfg.S2_frames * ifi * 1000);
    fprintf('  postS2Gap_frames = %d (%.1f ms)\n', cfg.postS2Gap_frames, cfg.postS2Gap_frames * ifi * 1000);
    fprintf('  ITI_frames       = %d (%.1f ms)\n', cfg.ITI_frames, cfg.ITI_frames * ifi * 1000);
    fprintf('  fixJitterFrames  = [%d %d]\n', cfg.fixJitterFrames(1), cfg.fixJitterFrames(2));

    if cfg.debug.quickTest
        fprintf('\nWARNING: quick-test mode enabled: cfg.calib.maxTrials = %d. Do not use for pilot/real EEG data.\n\n', cfg.calib.maxTrials);
    end
    ...
    cfg.calib.questPlus.stimDomain = cfg.minOpacityPct:1:cfg.maxOpacityPct;
    qp = cfg.calib.questPlus;

    questData = qpInitialize( ...
        'qpPF', @qpPF_blindSensingSeeingOpacity, ...
    ...
    tracks = repmat(struct( ...
        'name', '', 'targetOutcome', '', 'targetProb', 0, 'startOpacityPct', NaN, ...
        'xCurrent', NaN, 'xHistory', [], ...
        'nUpdates', 0, 'converged', false, ...
        'frozen', false, 'frozenOpacityPct', NaN), 1, 3);
    for k = 1:3
        ...
        tracks(k).startOpacityPct = cfg.calib.tracks(k).startOpacityPct;
        tracks(k).xCurrent      = cfg.calib.tracks(k).startOpacityPct;
    end
    ...
    fprintf('\nQUEST+ MOCS-informed setup (opacity %%):\n');
    for k = 1:3
        fprintf('Track %d %s: target=%s, targetProb=%.3f, startOpacityPct=%d, xCurrent=%d\n', ...
            k, tracks(k).name, tracks(k).targetOutcome, tracks(k).targetProb, ...
            tracks(k).startOpacityPct, tracks(k).xCurrent);
    end
    nAD = numel(qp.alphaDetRange); nBD = numel(qp.betaDetRange);
    nAL = numel(qp.alphaLocRange); nBL = numel(qp.betaLocRange);
    nL  = numel(qp.lambdaRange);
    fprintf('QUEST+ parameter grid: alphaDet=%d betaDet=%d alphaLoc=%d betaLoc=%d lambda=%d total=%d\n', ...
        nAD, nBD, nAL, nBL, nL, nAD*nBD*nAL*nBL*nL);
    ...
    practiceFileCSV = fullfile(outDir, sprintf('CB_4xGratings_Opacity_%s_PracticeFlow_%s.csv', cfg.participantID, timestamp));
    writetable(practiceTable, practiceFileCSV);
    fprintf('Saved practice flow CSV: %s\n', practiceFileCSV);
    practiceFileMAT = fullfile(outDir, sprintf('CB_4xGratings_Opacity_%s_PracticeFlow_%s.mat', cfg.participantID, timestamp));
    ...
    firstTrackUsePrinted = false(1, 3);
    questUpdateDisabledWarned = false;
    ...
    for t = 1:cfg.nTotal
        ...
        % Pick current opacity from assigned QUEST+ track (catch trials are tethered too).
        k = trackIndexFromStaircase(trial.staircase);
        if cfg.debug.forceFixedOpacity
            opacityPct = cfg.debug.fixedOpacityPct;
        elseif tracks(k).frozen
            opacityPct = tracks(k).frozenOpacityPct;
        else
            opacityPct = clamp(round(tracks(k).xCurrent), cfg.minOpacityPct, cfg.maxOpacityPct);
        end
        opacity01 = opacityPct / 100;

        if ~firstTrackUsePrinted(k)
            fprintf('First %s track trial used opacityPct=%d\n', tracks(k).name, opacityPct);
            firstTrackUsePrinted(k) = true;
        end
        ...
        drawGratings(window, gratingTex, allRects, oriS1, bg, opacity01);
       ...
       %drawMaskFixationOnly(window, bg, maskRect, cfg.stim.maskGrey, fixationCoords, cfg.fix.lineWidthPx, black, xCentre, yCentre);
        ...
        drawGratings(window, gratingTex, allRects, oriS2, bg, opacity01);
        drawFixation(window, fixationCoords, cfg.fix.lineWidthPx, black, xCentre, yCentre);
        ...
        vblTargetQ1 = tGap + (cfg.postS2Gap_frames - 0.5) * ifi;
        ...
        questUpdateDisabled = cfg.debug.forceFixedOpacity;
        if trial.isChange
            if hit == 0
                outcomeBin = 1;   % Blind
            elseif locCorrect == 1
            ...
            if ~questUpdateDisabled
                questData = qpUpdate(questData, opacityPct, outcomeBin);

                tracks(k).nUpdates = tracks(k).nUpdates + 1;
                tracks(k).xHistory(end+1) = opacityPct;
                ...
                if ~tracks(k).frozen && ...
                   ...
                   numel(tracks(k).xHistory) >= cfg.calib.convergenceWindow
                    ...
                    if (max(w) - min(w)) <= cfg.calib.convergenceTolOpacityPct
                        tracks(k).converged = true;
                        tracks(k).frozen = true;
                        tracks(k).frozenOpacityPct = clamp(round(tracks(k).xCurrent), ...
                            cfg.minOpacityPct, cfg.maxOpacityPct);
                        fprintf('Track %s froze at trial %d: opacityPct=%d\n', ...
                            tracks(k).name, t, tracks(k).frozenOpacityPct);
                    end
                end
            elseif ~questUpdateDisabledWarned
                fprintf('QUEST+ updating disabled (cfg.debug.forceFixedOpacity=true).\n');
                questUpdateDisabledWarned = true;
            end
        else
        ...
        results(t).opacityPct = opacityPct;
        results(t).opacity01 = opacity01;
        results(t).S1_frames = cfg.S1_frames;
        results(t).ISI_frames = cfg.ISI_frames;
        results(t).S2_frames = cfg.S2_frames;
        results(t).postS2Gap_frames = cfg.postS2Gap_frames;
        results(t).ITI_frames = cfg.ITI_frames;
        results(t).fixJitterFramesUsed = jitterFrames;
        results(t).questUpdateDisabled = double(questUpdateDisabled);
        ...
        results(t).track1_frozenOpacityPct = tracks(1).frozenOpacityPct;
        results(t).track2_frozenOpacityPct = tracks(2).frozenOpacityPct;
        results(t).track3_frozenOpacityPct = tracks(3).frozenOpacityPct;

        trialLogLine(t, cfg, trial, opacityPct, resp2, hit, locCorrect, pas, tracks);
        printTrialSummaryLine(t, cfg, trial, tracks(k).name, opacityPct, pas, locCorrect, ...
            results(t).outcomeBin, missedFixOn, missedS1, missedISI, missedS2, missedGap, missedQ1, missedQ2, missedITI);
    ...
    trackOpacityPct = nan(1,3);
    for kk = 1:3
        if isempty(tracks(kk).xHistory)
            trackOpacityPct(kk) = tracks(kk).startOpacityPct;
        else
            n = min(NRecent, numel(tracks(kk).xHistory));
            trackOpacityPct(kk) = round(median(tracks(kk).xHistory(end-n+1:end)));
        end
    end

    fprintf('Run estimates: %s=%d%%, %s=%d%%, %s=%d%%\n', ...
        tracks(1).name, trackOpacityPct(1), ...
        tracks(2).name, trackOpacityPct(2), ...
        tracks(3).name, trackOpacityPct(3));
    ...
    expLabels = {sprintf('%d (S1)', cfg.S1_frames), ...
        ...
        sprintf('%d (gap)', cfg.postS2Gap_frames)};
    ...
    T.track1_finalOpacityPct = repmat(trackOpacityPct(1), height(T), 1);
    T.track2_finalOpacityPct = repmat(trackOpacityPct(2), height(T), 1);
    T.track3_finalOpacityPct = repmat(trackOpacityPct(3), height(T), 1);
    ...
    T.track1_frozenOpacityPctFinal = repmat(tracks(1).frozenOpacityPct, height(T), 1);
    T.track2_frozenOpacityPctFinal = repmat(tracks(2).frozenOpacityPct, height(T), 1);
    T.track3_frozenOpacityPctFinal = repmat(tracks(3).frozenOpacityPct, height(T), 1);

    T.track1_startOpacityPct = repmat(cfg.calib.tracks(1).startOpacityPct, height(T), 1);
    T.track2_startOpacityPct = repmat(cfg.calib.tracks(2).startOpacityPct, height(T), 1);
    T.track3_startOpacityPct = repmat(cfg.calib.tracks(3).startOpacityPct, height(T), 1);
    ...
    cal.tracks         = tracks;
    cal.trackOpacityPct = trackOpacityPct;
    ...
    calFile = fullfile(outDir, sprintf('CB_4xGratings_Opacity_%s_FullRun_%s.mat', cfg.participantID, timestamp));
    ...
    if ~isempty(flipLog)
        ...
        flipLogFile = fullfile(outDir, sprintf('CB_4xGratings_Opacity_%s_FlipLog_%s.csv', cfg.participantID, timestamp));
    ...
    if exist('triggerLog', 'var') && ~isempty(triggerLog)
        ...
        triggerLogFile = fullfile(outDir, sprintf('CB_4xGratings_Opacity_%s_TriggerLog_%s.csv', cfg.participantID, timestamp));
    ...
    function emitTrigger(label, context, trialNum, blockNum, code, linkedFlipLabel, linkedFlipTime)
        ...
        try
            if isfield(cfg, 'trigger') && cfg.trigger.enabled && ~isempty(cfg.trigger.handle)
                ...
                didEnable = true;
                % resetMode 'blocking' / zero reset deferred to a later phase
            end
        catch ME
...
function showPractice1BeginScreen(window, windowRect, bg, black, cfg) %#ok<INUSD>
    ...
    Screen('TextStyle', window, 0);
    cfg.loggedFlip('practice1_begin', 'practice1_begin', NaN, NaN, NaN);
...
function showPractice2BeginScreen(window, windowRect, bg, black, cfg) %#ok<INUSD>
    ...
    DrawFormattedText(window, titleTxt, 'center', 'center', black);
    Screen('TextStyle', window, 0);
    cfg.loggedFlip('practice2_begin', 'practice2_begin', NaN, NaN, NaN);
...
function showCalibrationIntroScreen(window, windowRect, bg, black, cfg)
    ...
    [tOn, ~, ~, ~] = cfg.loggedFlip('calibration_intro_grey', 'calibration_intro', NaN, NaN, NaN);
    ...
    Screen('TextSize', window, tcfg.promptSize);
    DrawFormattedText(window, 'Press SPACEBAR to continue.', 'center', promptY, black);
    cfg.loggedFlip('calibration_intro_active', 'calibration_intro', NaN, NaN, NaN);
...
function showMainTrialBeginScreen(window, windowRect, bg, black, cfg) %#ok<INUSD>
    ...
    DrawFormattedText(window, titleTxt, 'center', 'center', black);
    Screen('TextStyle', window, 0);
    cfg.loggedFlip('main_begin', 'main_begin', NaN, NaN, NaN);
...
function showBlockResumeScreen(window, windowRect, bg, black, cfg) %#ok<INUSD>
    ...
    DrawFormattedText(window, titleTxt, 'center', 'center', black);
    Screen('TextStyle', window, 0);
    cfg.loggedFlip('block_resume', 'block_resume', NaN, NaN, NaN);
...
function printPracticeSummary(summary, label)
    ...
    if isfield(summary,'nTrials')
        ...
        if isfield(summary,'nSTD') && isfield(summary,'nNCH') && isfield(summary,'nEASY')
            if isfield(summary,'nHARD')
                fprintf('  (STD=%d, NCH=%d, EASY=%d, HARD=%d)', summary.nSTD, summary.nNCH, summary.nEASY, summary.nHARD);
            else
...
function probs = qpPF_blindSensingSeeingOpacity(stim, params)
    % Multinomial psychometric function for the QUEST+ calibration.
    % In this script, QUEST+ x = opacity percentage.
    % Inputs:
    %   stim   - [nStim x 1] stimulus opacity percentage (1-100)
    ...
    stim = stim(:);
    if size(params, 2) ~= 5
        error('qpPF_blindSensingSeeingOpacity: params must have 5 columns [aD bD aL bL lambda].');
    end
...
function xNext = selectXForTrack(track, questData, cfg)
    % Choose next x for this track: the opacity percentage in cfg.calib.questPlus.stimDomain
    ...
    probs = qpPF_blindSensingSeeingOpacity(stimDomain, paramsMAP);
    pBlind   = probs(:,1,1);
    pSensing = probs(:,1,2);
    pSeeing  = probs(:,1,3);
...
function pTrials = buildPracticeTrialList(pracCfg, allowedOri)
    ...
    for i = 1:nSTD
        ...
        pTrials(k).opacityPct = pracCfg.opacitySTD;
        k = k + 1;
    end
    ...
    for i = 1:nNCH
        ...
        pTrials(k).opacityPct = pracCfg.opacityNCH;
        k = k + 1;
    end
    ...
    for i = 1:nEASY
        ...
        pTrials(k).opacityPct = pracCfg.opacityEASY;
        k = k + 1;
    end

    % ---------------- HARD (faint change) ----------------
    nHARD = pracCfg.nHARD;
    hardQuads = makeBalancedQuads(nHARD);

    for i = 1:nHARD
        pTrials(k).trialType  = 'HARD';
        pTrials(k).isChange   = true;
        pTrials(k).changeQuad = hardQuads(i);
        pTrials(k).opacityPct = pracCfg.opacityHARD;
        k = k + 1;
    end
...
function drawGratings(window, tex, allRects, orientations, bg, opacity01)
    if nargin < 6 || isempty(opacity01)
        opacity01 = 1;
    end

    opacity01 = max(0, min(1, opacity01));

    Screen('FillRect', window, bg);

    for i = 1:4
        Screen('DrawTexture', window, tex, [], allRects(i,:), orientations(i), [], opacity01);
    end
end
...
function r = emptyResultRow()
    r = struct( ...
        ...
        'ISI_frames', NaN, ...
        'S2_frames', NaN, ...
        'postS2Gap_frames', NaN, ...
        'ITI_frames', NaN, ...
        'fixJitterFramesUsed', NaN, ...
        'questUpdateDisabled', NaN, ...
        'oriS1', '', ...
        'oriS2', '', ...
        'tS1', NaN, ...
        'tISI', NaN, ...
        ...
        'track2_frozen', NaN, ...
        'track3_frozen', NaN, ...
        'track1_frozenOpacityPct', NaN, ...
        'track2_frozenOpacityPct', NaN, ...
        'track3_frozenOpacityPct', NaN, ...
        'tTrialStart', NaN, ...
        'tTrialEnd',   NaN, ...
        'trialTotalSec', NaN ...
    );
...
function cleanup(~, cfg)
    try
        ...
        if isfield(cfg, 'qTex')
            try
                Screen('Close', [cfg.qTex.PAS, cfg.qTex.Loc_default, cfg.qTex.Loc_pas1]);
            catch
            end
        end
...
function trialLogLine(t, cfg, trial, opacityPct, resp2, hit, locCorrect, pas, tracks)
    ...
    % --- Per-trial line ---
    fprintf(['blk=%02d trl=%03d %s st=%s q=%s opacity=%02d | Q1=%s Q2=%s | hit=%d loc=%s | score=%.1f | ' ...
...
function printTrialSummaryLine(t, cfg, trial, trackName, opacityPct, pas, locCorrect, outcomeStr, ...
    missedFixOn, missedS1, missedISI, missedS2, missedGap, missedQ1, missedQ2, missedITI)

    if ~isfield(cfg,'debug') || ~isfield(cfg.debug,'printTrialSummary') || ~cfg.debug.printTrialSummary
        return;
    end
    ...
    missFlags = [missedFixOn missedS1 missedISI missedS2 missedGap missedQ1 missedQ2 missedITI];
    missFlags(isnan(missFlags)) = 0;
    nMisses = sum(missFlags > 0);

    fprintf('blk=%02d trl=%03d %s track=%s opacityPct=%s PAS=%s locCorrect=%s outcome=%s S1=%d ISI=%d S2=%d gap=%d ITI=%d misses=%d\n', ...
        blockNum, t, tc, trackStr, sprintf('%d', opacityPct), pasStr, locStr, outcomeStr, ...
        cfg.S1_frames, cfg.ISI_frames, cfg.S2_frames, cfg.postS2Gap_frames, cfg.ITI_frames, nMisses);
end
...
function summary = runPracticeBlock(window, windowRect, gratingTex, allRects, fixationCoords, xCentre, yCentre, ifi, cfg, bg, black, pracCfg)
    ...
    % --- Build mixed practice trial list (STD / NCH / EASY / HARD) ---
    pTrials = buildPracticeTrialList(pracCfg, allowedOri);

    isPractice2 = strcmp(pracCfg.name, 'Practice Block 2');
    doPracticeTriggers = isPractice2 && ...
        isfield(cfg.eeg.markerPolicy, 'markPracticeBlock2') && ...
        cfg.eeg.markerPolicy.markPracticeBlock2;
    ...
    m.nHARD = 0; m.nHARD_detect = 0; m.nHARD_see = 0;
    ...
    m.nChange = 0; m.nChange_detect = 0; m.nChange_see = 0;
        ...
        for p = 1:numel(pTrials)
            ...
            if doPracticeTriggers
                cfg.emitTrigger('trialStart', pracCfg.name, p, NaN, cfg.eeg.codes.trialStart, 'FixOn', tFixOn);
            end
            tTrialStart = tFixOn;
            ...
            if doPracticeTriggers
                cfg.emitTrigger('s1On', pracCfg.name, p, NaN, cfg.eeg.codes.s1On, 'S1', tS1);
            end
            ...
            if doPracticeTriggers
                cfg.emitTrigger('isiOn', pracCfg.name, p, NaN, cfg.eeg.codes.isiOn, 'ISI', tISI);
            end
            ...
            if doPracticeTriggers
                cfg.emitTrigger('s2On', pracCfg.name, p, NaN, cfg.eeg.codes.s2On, 'S2', tS2);
            end
            ...
            if doPracticeTriggers
                cfg.emitTrigger('gapOn', pracCfg.name, p, NaN, cfg.eeg.codes.gapOn, 'Gap', tGap);
            end
            ...
            Screen('DrawTexture', window, cfg.qTex.PAS);
            vblTargetQ1 = tGap + (cfg.postS2Gap_frames - 0.5) * ifi;
            [tQ1, ~, ~, missedQ1] = cfg.loggedFlip('Q1_PAS', pracCfg.name, p, NaN, vblTargetQ1);
            if doPracticeTriggers
                cfg.emitTrigger('q1On', pracCfg.name, p, NaN, cfg.eeg.codes.q1On, 'Q1_PAS', tQ1);
            end
            ...
            if doPracticeTriggers && cfg.eeg.markerPolicy.markResponses
                if ~isnan(pas) && pas >= 1 && pas <= 4
                    cfg.emitTrigger('pasResponse', pracCfg.name, p, NaN, cfg.eeg.codes.pasBase + pas, 'Q1_PAS', tQ1);
                end
            end
            ...
            if doPracticeTriggers
                cfg.emitTrigger('q2On', pracCfg.name, p, NaN, cfg.eeg.codes.q2On, 'Q2_Loc', tQ2);
            end
            [resp2Key, ~] = waitForKeyQueue(cfg.keys.quad, cfg.keys.escape, cfg.maxRespSec, cfg);
            resp2 = keyToMappedValue(resp2Key, cfg.keys.quad, cfg.keys.quadValues);
            if doPracticeTriggers && cfg.eeg.markerPolicy.markResponses
                if ~isnan(resp2) && resp2 >= 1 && resp2 <= 4
                    cfg.emitTrigger('locResponse', pracCfg.name, p, NaN, cfg.eeg.codes.locBase + resp2, 'Q2_Loc', tQ2);
                end
            end
            ...
            if isfield(cfg,'practice') && isfield(cfg.practice,'logToCommandWindow') && cfg.practice.logToCommandWindow
                ...
                fprintf('PRACTICE %-14s trl=%02d/%02d type=%-4s q=%s opacity=%02d | PAS=%s Q2=%s | det=%d loc=%s | %s\n', ...
                    pracCfg.name, p, numel(pTrials), trial.trialType, qStr, opacityPct, pasStr, ...
            ...
            switch trial.trialType
                ...
                case 'HARD'
                    m.nHARD = m.nHARD + 1;
                    if detected
                        m.nHARD_detect = m.nHARD_detect + 1;
                    end
                    if detected && locCorrect == 1
                        m.nHARD_see = m.nHARD_see + 1;
                    end
            end
            ...
            if doPracticeTriggers
                cfg.emitTrigger('trialEnd', pracCfg.name, p, NaN, cfg.eeg.codes.trialEnd, 'ITI', tITI);
            end
            holdForSecondsWithAbort(tITI + cfg.ITI_frames*ifi, cfg);
        end
        ...
        summary.nHARD_detect = m.nHARD_detect;
        summary.nHARD_see    = m.nHARD_see;
        ...
        summary.hardDetectRate = safeRate(m.nHARD_detect, m.nHARD);
        summary.hardSeeRate    = safeRate(m.nHARD_see, m.nHARD);
