function report = CB_EEG_RunFullPreprocessingPipeline_ALEX01(varargin)
...
if ~isnan(locMismatch) && locMismatch ~= 0
    error('Localisation response mismatch detected between ALEX01 EEG triggers and behavioural CSV.');
end
...
for t = 1:n
    ...
    if isempty(s2Rel) || isempty(gapRel) || isempty(q1Rel) || isempty(pasRel) || isempty(locRel)
        error('ALEX01 trial %d has an incomplete surviving trigger chain.', t);
    end
    if ~(s2Rel < gapRel && gapRel < q1Rel && q1Rel < pasRel && pasRel < locRel)
        error('ALEX01 trial %d surviving trigger order is invalid.', t);
    end
...
audit = table(trial, repmat(string(timingSource),n,1), s1DurationSec, ...
    ...
    trials.pasCode, trials.pasResp, trials.locCode, trials.locResp, ...
    ...
    'PASCode','PASResp','LocCode','LocResp','EEG_ISI_to_S2_ms','CSV_ISI_to_S2_ms', ...
